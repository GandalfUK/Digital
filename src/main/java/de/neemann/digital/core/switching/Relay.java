...
public class Relay extends Node implements Element {
    ...
    private static final class Pole {
        ...
        private Pole(int bits, int num) {
            ...
            s = new Switch(outputAB, outputB, false);
        }

        private void addOutputs(ObservableValues.Builder ov) {
            ov.add(outputAB, outputB);
        }

        public void setInputs(ObservableValue inAB, ObservableValue inB) throws NodeException {
            s.setInputs(new ObservableValues(inAB, inB));
        }
